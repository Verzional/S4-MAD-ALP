...
struct ColorMixingView: View {
    ...
    private var headerSection: some View {
        ...
        .overlay(
            Rectangle()
                .fill(Color(hex:"363638").opacity(0.2))
                .frame(height: 1),
            alignment: .bottom
        )
    }
    ...
    private func selectionSlot(color: ColorItem?, slotNumber: Int) -> some View {
        VStack(spacing: 8) {
            ZStack {
                Circle()
                    .fill(color != nil ? Color(hex: color!.hex) : Color(hex:"363638").opacity(0.2))
    ...
    private func slotBorder(hasColor: Bool) -> some View {
        Circle()
            .stroke(hasColor ? Color.clear : Color(hex:"363638").opacity(0.3),
